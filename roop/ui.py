...
def create_root(start: Callable[[], None], destroy: Callable[[], None]) -> ctk.CTk:
    ...
    source_button = ctk.CTkButton(root, text='选择人脸', cursor='hand2', command=lambda: select_source_path())
    source_button.place(relx=0.1, rely=0.4, relwidth=0.3, relheight=0.1)

    target_button = ctk.CTkButton(root, text='选择替换视频', cursor='hand2', command=lambda: select_target_path())
    target_button.place(relx=0.6, rely=0.4, relwidth=0.3, relheight=0.1)

    keep_fps_value = ctk.BooleanVar(value=roop.globals.keep_fps)
    keep_fps_checkbox = ctk.CTkSwitch(root, text='保留目标帧率', variable=keep_fps_value, cursor='hand2',
                                      command=lambda: setattr(roop.globals, 'keep_fps', not roop.globals.keep_fps))
    # keep_fps_checkbox.place(relx=0.1, rely=0.6)
    keep_fps_checkbox.place(relx=0.1, rely=0.6)

    keep_frames_value = ctk.BooleanVar(value=roop.globals.keep_frames)
    keep_frames_switch = ctk.CTkSwitch(root, text='保留临时相框', variable=keep_frames_value, cursor='hand2',
                                       command=lambda: setattr(roop.globals, 'keep_frames', keep_frames_value.get()))
    # keep_frames_switch.place(relx=0.1, rely=0.65)
    keep_frames_switch.place(relx=0.6, rely=0.6)
    # skip_audio_value = ctk.BooleanVar(value=roop.globals.skip_audio)
    # skip_audio_switch = ctk.CTkSwitch(root, text='Skip target audio', variable=skip_audio_value, cursor='hand2',
    #                                   command=lambda: setattr(roop.globals, 'skip_audio', skip_audio_value.get()))
    # skip_audio_switch.place(relx=0.6, rely=0.6)
    #
    # many_faces_value = ctk.BooleanVar(value=roop.globals.many_faces)
    # many_faces_switch = ctk.CTkSwitch(root, text='Many faces', variable=many_faces_value, cursor='hand2',
    ...
    stop_button = ctk.CTkButton(root, text='结束', cursor='hand2', command=lambda: destroy())
...
def select_output_path(start: Callable[[], None]) -> None:
    ...
    if is_image(roop.globals.target_path):
        output_path = ctk.filedialog.asksaveasfilename(title='save image output file', defaultextension='.png',
                                                       initialfile='output.png', initialdir=RECENT_DIRECTORY_OUTPUT)
    elif is_video(roop.globals.target_path):
        output_path = ctk.filedialog.asksaveasfilename(title='save video output file', defaultextension='.mp4',
                                                       initialfile='output.mp4', initialdir=RECENT_DIRECTORY_OUTPUT)
    else:
        output_path = None
    if output_path:
